...
// Updated notifications for Indian startups
let notifications = [
  { name: "Zepto raised $665M", description: "Quick Commerce", icon: "Z", color: "#FF5A5F" },
  { name: "PhysicsWallah hits $1.1B", description: "EdTech", icon: "P", color: "#6C5CE7" },
  { name: "Meesho joins unicorn club", description: "Social Commerce", icon: "M", color: "#00CEC9" },
  { name: "Krutrim: India’s first AI unicorn", description: "AI", icon: "K", color: "#FD79A8" },
];
...
const Bento = () => {
  return (
    <div id="features" className="flex flex-col w-full">
      <Header title="Why This Directory Stands Out" />
          ...
          <div className="border border-dashed border-[#252525] w-full h-full relative overflow-hidden p-[30px] flex-shrink-0 flex flex-col gap-4 lg:gap-0 ">
            <div className="space-y-1 flex-shrink-0 z-10">
              <motion.h5
                ...
                animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                transition={{ duration: 0.6, ease: 'easeOut', type: 'spring', delay: 0.35 }}
                className='text-2xl lg:text-3xl tracking-tighter text-left'>Latest Funding Updates</motion.h5>
              <motion.p
                initial={{ opacity: 0, y: 30, filter: 'blur(10px)' }}
                animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                transition={{ duration: 0.6, ease: 'easeOut', type: 'spring', delay: 0.35 }}
                className='tracking-tight text-sm lg:text-lg text-[#d1d1d1]'>Real-time funding rounds, valuations, and investor news.</motion.p>
            </div>
            <div className="h-full overflow-hidden w-full flex items-start lg:pt-10 lg:pb-4 relative z-10">
              <AnimatedList className='w-full'>
                {notifications.map((item, idx) => (
            ...
            <div className="absolute w-full h-full top-0 left-0">
              <Image src="/assets/mask.svg" alt="background" fill className="object-cover w-full h-full opacity-60 " />
            </div>
            <div className="absolute h-full w-full bg-gradient-to-t from-[#101010]/75 via-transparent to-[#101010]/75 top-0 left-1/2 -translate-x-1/2"></div>
            <div className="absolute h-full w-full bg-gradient-to-r from-[#101010]/75 via-transparent to-[#101010]/75 top-0 left-1/2 -translate-x-1/2"></div>
            <div className="h-[90px] absolute left-1/2 -translate-x-1/2 -bottom-7 w-full bg-[#101010] blur-[15px] z-20"></div>
          ...
          <div className="border border-dashed border-[#252525] w-full h-full relative overflow-hidden p-[30px] flex-shrink-0 space-y-1 flex flex-col items-center justify-start gap-4 z-10">
            <div className="w-full z-10">
              <motion.h5
                ...
                animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                transition={{ duration: 0.6, ease: 'easeOut', type: 'spring', delay: 0.35 }}
                className='text-2xl lg:text-3xl tracking-tighter text-left'>Instant Search</motion.h5>
              <motion.p
                initial={{ opacity: 0, y: 30, filter: 'blur(10px)' }}
                animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                transition={{ duration: 0.6, ease: 'easeOut', type: 'spring', delay: 0.35 }}
                className='tracking-tight text-sm lg:text-lg text-[#d1d1d1]'>Find any Indian startup by name, sector, or city in milliseconds.</motion.p>
              </div>
              <div className="absolute w-full h-full top-0 left-0">
                <Image src="/assets/mask.svg" alt="background" fill className="object-cover w-full h-full opacity-60 " />
              </div>
            ...
            <div className="border border-dashed border-[#252525] w-full h-full relative overflow-hidden p-[30px] flex-shrink-0 flex flex-col gap-4 lg:gap-0 ">
              <div className="space-y-1 flex-shrink-0 z-10">
                <motion.h5
                  ...
                  animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                  transition={{ duration: 0.6, ease: 'easeOut', type: 'spring', delay: 0.35 }}
                  className='text-2xl lg:text-3xl tracking-tighter text-left'>Powerful Filters</motion.h5>
                <motion.p
                  initial={{ opacity: 0, y: 30, filter: 'blur(10px)' }}
                  animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                  transition={{ duration: 0.6, ease: 'easeOut', type: 'spring', delay: 0.35 }}
                  className='tracking-tight text-sm lg:text-lg text-[#d1d1d1]'>Filter by sector, funding amount, HQ city, or founding year.</motion.p>
                </div>
                <div className="h-full w-full flex items-center lg:pt-10 lg:pb-4 relative overflow-hidden z-20">
                  <CardStack />
                </div>
                <div className="absolute w-full h-full top-0 left-0">
                  <Image src="/assets/cal.svg" alt="background" fill className="object-cover w-full h-full opacity-60 " />
                </div>
                <div className="absolute h-full w-full bg-gradient-to-t from-[#101010]/75 via-transparent to-[#101010]/75 top-0 left-1/2 -translate-x-1/2"></div>
                <div className="absolute h-full w-full bg-gradient-to-r from-[#101010]/75 via-transparent to-[#101010]/75 top-0 left-1/2 -translate-x-1/2"></div>
              </div>
...
const Notification = ({ name, description, icon, color }: any) => {
  return (
    <figure className={cn(
      ...
      <div className="flex flex-row items-center gap-3">
        <div className="flex size-10 items-center justify-center rounded-2xl overflow-hidden relative" style={{ backgroundColor: color || "#333" }}>
          <span className="text-white font-bold text-xl">{icon}</span>
        </div>
        <div className="flex flex-col overflow-hidden">
          <figcaption className="text-sm sm:text-lg font-medium text-white">{name}</figcaption>
